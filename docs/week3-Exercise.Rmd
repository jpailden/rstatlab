...
```{r}
install.packages("mosaic") # download the package - need internet connection
...
2. The data set `MLB-TeamBatting-S16.csv` contains MLB Team Batting Data for selected variables. Load the data set from the given url using the code below. This data set was obtained from [Baseball Reference](https://www.baseball-reference.com/leagues/MLB/2016-standard-batting.shtml).
    ...
    * SF - Sacrifice Flies  
    ...
    iii) Using your own words, describe the distribution of each variable in (i). Write your answer outside the code chunck. 
    iv) Find the average and the standard deviation of the variables `RPG`, `H` and `BA` for each league. Describe any differences or similarities between the leagues. Write your comment outside the code chunk.
