...
```
    ...
    iii) Use `group_by` to group the rows by `exerany` (exercise any). Repeat part (ii) on this grouped data. Comment on what you observe in the average weights between groups.
    
    vi) Repeat part (iii) but now use the grouping variables `smoke100` and `gender`. Comment on what you observe in the average weights between groups.  
    
    v) Obtain a random sample of 1000 rows and save this into `csc.samp1`. 
    
    vi) Repeat parts (ii) to (v) but only using the subset data `csc.samp1`.  
    
    vii) Comment on what you difference you observe (if any) between the results in the original sample and the smaller sample?
...
3. Use `sample()` to generate rolls from biased coint with $Pr(Head) = 0.6$ .
